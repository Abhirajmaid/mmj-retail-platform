  ...
  return (
    <div className="min-w-0 space-y-4 sm:space-y-6">
      <PageHeader
        ...
        description="Fill payment details in stock-add style workflow."
        actions={
          <Button
            type="button"
            variant="outline"
      ...
      <Card className="min-w-0" padding="lg">
        <CardHeader className="flex flex-row items-start gap-3 border-b border-zinc-100 pb-4">
          ...
          <div className="min-w-0 space-y-1">
            <CardTitle className="text-lg font-semibold text-zinc-900">Customer & Payment Header</CardTitle>
            <p className="text-sm text-zinc-500">Customer, amount, date, method and status</p>
          </div>
        </CardHeader>
        <CardBody className="space-y-4 pt-0">
          <div className="grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-zinc-900">Payment Date</label>
              <input
                type="date"
                className={inputClass}
      ...
      <Card className="min-w-0" padding="lg">
        <CardHeader className="flex flex-row items-start gap-3 border-b border-zinc-100 pb-4">
          ...
          <div className="min-w-0 space-y-1">
            <CardTitle className="text-lg font-semibold text-zinc-900">Payment Information</CardTitle>
            <p className="text-sm text-zinc-500">Invoice number, method, reference and notes</p>
          </div>
        </CardHeader>
        <CardBody className="space-y-4 pt-0">
          <div className="grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
              ...
              <select
                className={selectClass}
                value={form.method}
                onChange={(e) => setForm((c) => ({ ...c, method: e.target.value }))}
              >
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-zinc-900">Reference</label>
              <input
                className={inputClass}
                placeholder="Reference"
                value={form.reference}
                onChange={(e) => setForm((c) => ({ ...c, reference: e.target.value }))}
              />
            </div>
            <div>
          ...
          <div className="rounded-lg border border-zinc-200 bg-amber-50/30 p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-zinc-500">Payment Preview Amount</p>
      ...
      <div className="flex flex-col gap-3 border-t border-zinc-100 bg-white pt-6 sm:flex-row sm:items-center sm:justify-between">
        <Link href="/payments" className="order-2 sm:order-1">
